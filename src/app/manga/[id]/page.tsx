...
import { addChaptersToManga, MangaFactory, storyToManga } from '@/lib/data';
...
export default function MangaDetailPage({ params }: { params: { id: string } }) {
  ...
  const { user } = useAuth();
  const { story, isLoading: storyLoading, error: storyError } = useBackendStory(storyId);
  ...
  const readingChapter = useMemo(() => {
    ...
    return chapters.find((chapter) => chapter.id === progress.chapterId) ?? null;
  }, [chapters, progress]);
  ...
  const { subscribed, toggle: toggleSubscription } = useBackendSubscription(storyId);
      ...
      <main className="animate-in">
        ...
        >
          ...
          >
            ...
            >
              {genreIcon} {MangaFactory.getLabelEN(manga.genre)} · {manga.status}
            </span>
            ...
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
              <button
                ...
                onClick={() => {
                  const targetChapterId = progress?.chapterId ?? chapters[0]?.id;
                  if (!targetChapterId) return;
                  router.push(`/manga/${manga.id}/read?chapterId=${targetChapterId}`);
                }}
              >
                {readingChapter ? `Đọc tiếp Ch.${readingChapter.chapterNumber}` : 'Đọc từ đầu'} →
              </button>
              <button className={`btn ${saved ? 'btn-danger' : ''}`} onClick={toggle}>
              ...
              <button
                ...
                aria-label={subscribed ? 'Hủy theo dõi truyện' : 'Theo dõi truyện'}
              >
        ...
        >
          ...
          <div style={{ borderRight: '2px solid var(--ink)' }}>
            ...
            {manga.chapters.map((ch, i) => {
              const isRead = readingChapterIndex >= 0 && i < readingChapterIndex;
              ...
              return (
                <div
                  ...
                  onClick={() => {
                    if (!chapterId) return;
                    router.push(`/manga/${manga.id}/read?chapterId=${chapterId}`);
                  }}
                  onKeyDown={(e) => {
                ...
                >
                  ...
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: '0.87rem', fontWeight: 700 }}>{ch.title}</div>
                    <div
                      style={{
                        fontFamily: "'IBM Plex Mono', monospace",
                        fontSize: '0.62rem',
          ...
          <div>
            {/* Rating */}
            <div style={{ padding: '20px', borderBottom: '2px solid var(--ink)' }}>
              <div
                ...
                {[1, 2, 3, 4, 5].map((s) => (
                  <button
                    ...
                    style={{
                      ...
                      cursor: 'pointer',
                      lineHeight: 1,
                      color: (hoverStar || rating) >= s ? accent : 'var(--aged)',
