...
import { apiClient, getToken, normalizePaginatedA, normalizePaginatedB } from '@/lib/api';
...
export function useBackendChapters(storyId: number | null, page = 1, limit = 20) {
...
export function useBackendProgress(storyId: number) {
  ...
  const save = useCallback(
    async (chapterId: number, scrollPosition: number, readingMode: BackendReadMode) => {
      const userId = getUserId();
      if (!userId) return;
...
export function useBackendReadingMode(storyId?: number) {
  ...
  const setMode = useCallback(
    async (newMode: BackendReadMode) => {
      const userId = getUserId();
      if (!userId) return;
      const savedMode = await mgr.saveBackendReadingMode(userId, newMode, storyId);
