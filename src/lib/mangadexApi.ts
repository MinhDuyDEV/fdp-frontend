...
export async function fetchChapters(mangaId: string, limit = 10): Promise<MdxChapterInfo[]> {
  ...
  if (!data.data) return [];
  return data.data.map((c: { id: string; attributes: Record<string, unknown> }) => ({
    id: c.id,
    chapter: (c.attributes.chapter as string) ?? '?',
    title: c.attributes.title as string | null,
    pages: c.attributes.pages as number,
    translatedLanguage: c.attributes.translatedLanguage as string,
  }));
}
